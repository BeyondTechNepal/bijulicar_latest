...
class BusinessDirectoryController extends Controller
{
    public function index(Request $request)
    {
        ...
        $businesses = $query->get()->map(function ($user) {
            ...
            return [
                ...
                'rentable_listings'=> $rentableCars->count(),
        ...
        if ($sort === 'rating')        $businesses = $businesses->sortByDesc('avg_rating');
        elseif ($sort === 'reviews')   $businesses = $businesses->sortByDesc('review_count');
        else                           $businesses = $businesses->sortByDesc('active_listings');

        $totalBusinesses   = $businesses->count();
        $totalListings     = $businesses->sum('active_listings');
        $totalRentable     = $businesses->sum('rentable_listings');
        ...
        return view('frontend.pages.businesses', compact(
            'businesses', 'totalBusinesses', 'totalListings', 'totalRentable',
            'totalCities', 'latestNews', 'cities', 'businessBannerAds'
        ));
    }
    ...
    public function show(int $id)
    {
        ...
        $rentableCars = $activeCars->whereIn('listing_type', ['rent', 'both'])->values();
        ...
        return view('frontend.pages.business_profile', compact(
            'user', 'activeCars', 'rentableCars', 'allReviews',
            'avgRating', 'reviewCount', 'businessName', 'contact', 'email',
            'mapLocation', 'spec', 'location', 'businessNews', 'businessProfileAds'
        ));
    }
}