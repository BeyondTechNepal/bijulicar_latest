...
@section('title', 'Edit Admin Permission')
    ...
    <div class="max-w-2xl">
        ...
        <div class="bg-white border border-slate-200 rounded-[2rem] shadow-sm overflow-hidden">
            ...
            <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                <div>
                    <h3 class="text-base font-black text-slate-900 tracking-tight">Edit Admin Authorization Token</h3>
                    <p class="text-[11px] text-slate-500 font-medium mt-0.5">
                        Updating this key affects the <span class="text-indigo-600 font-bold">admin</span> guard globally.
                    </p>
                </div>
                <div class="text-right">
                    <span class="block text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none">Record
                        ID</span>
                    <span class="text-sm font-mono font-bold text-slate-400">#{{ $permission->id }}</span>
                </div>
            </div>

            <form method="POST" action="{{ route('admin.admin_permissions.update', $permission) }}" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    <div class="flex justify-between items-center">
                        <label for="name" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest">
                            Permission Key Name
                        </label>
                        <span
                            class="text-[9px] font-bold text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">
                            Immutable Guard: Admin
                        </span>
                    </div>
                ...
                <div class="p-4 bg-slate-50 rounded-xl border border-slate-100 flex gap-3">
                    ...
                    <p class="text-[11px] text-slate-500 leading-relaxed">
                        Changing the name of an admin permission can affect backend functionality. Update your application
                        logic accordingly.
                    </p>
                </div>
