...
@section('title', 'Create New Admin Role')
...
@section('content')
    <div class="max-w-3xl">
        ...
        <a href="{{ route('admin.admin_roles.index') }}"
            class="inline-flex items-center text-xs font-black text-slate-400 hover:text-indigo-600 transition-colors mb-6 group uppercase tracking-widest">
        ...
        <div class="bg-white border border-slate-200 rounded-[2.5rem] shadow-sm overflow-hidden">
            ...
            <div class="px-10 py-8 border-b border-slate-100 bg-slate-50/50">
                <h3 class="text-xl font-black text-slate-800 tracking-tight">Initialize Admin Role</h3>
                <p class="text-xs text-slate-500 font-medium mt-1">Define a new admin role and assign its baseline
                    permissions for the <span class="text-indigo-600 font-bold">admin</span> guard.</p>
            </div>

            <form method="POST" action="{{ route('admin.admin_roles.store') }}" class="p-10 space-y-10">
                @csrf

                {{-- Role Name Input --}}
                <div class="space-y-3">
                    <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Unique Role
                        Identifier</label>
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                        <label
                            class="block text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Authorization
                            Matrix</label>
                    ...
                    @if ($permissions->isEmpty())
                        <div class="py-12 text-center border-2 border-dashed border-slate-100 rounded-3xl bg-slate-50/30">
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">No permission tokens
                                found</p>
                            <a href="{{ route('admin.permissions.create') }}"
                                class="text-[10px] font-black text-indigo-600 hover:text-indigo-800 underline underline-offset-4 uppercase">Create
                                Tokens First</a>
                        </div>
                    @else
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            @foreach ($permissions as $permission)
                                ...
                                <label
                                    class="group relative flex items-center gap-3 p-4 border rounded-2xl cursor-pointer transition-all duration-200
                                  {{ $isOld ? 'border-indigo-500 bg-indigo-50/50' : 'border-slate-100 hover:border-slate-300 hover:bg-slate-50' }}">
                                    ...
                                    <span
                                        class="text-xs font-mono font-bold tracking-tight {{ $isOld ? 'text-indigo-900' : 'text-slate-600' }}">
                                        {{ $permission->name }}
                                    </span>
                                </label>
                            @endforeach
