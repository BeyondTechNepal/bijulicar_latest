        ...
        <form method="POST"
            ...
            <div class="bg-slate-50 rounded-[2rem] p-8 space-y-6 border border-slate-100">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span class="ml-auto text-[9px] text-slate-400 font-medium italic">Click the map to drop a pin</span>
                </div>

                {{-- Interactive Leaflet Map Picker --}}
                <div id="picker-map" class="w-full h-64 rounded-2xl border border-slate-200 overflow-hidden" style="z-index:0"></div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Latitude</label>
                        <input type="text" name="latitude" id="lat-input" value="{{ old('latitude', $location->latitude ?? '') }}" required placeholder="27.7172"
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="longitude" id="lng-input" value="{{ old('longitude', $location->longitude ?? '') }}" required placeholder="85.3240"
                            class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-mono font-bold text-slate-600 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all">
                    </div>
                </div>
                ...
                <div class="space-y-2 pt-2">
                    ...
                    <input type="text" name="address" id="address-input" value="{{ old('address', $location->address ?? '') }}" required placeholder="123 Digital Ave, Suite 400"
