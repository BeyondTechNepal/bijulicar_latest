...
@section('content')
    ...
    <div class="flex items-center gap-3 mb-5 flex-wrap">
        ...
        <span class="text-[10px] font-black px-2.5 py-1 rounded-full bg-amber-100 text-amber-700 uppercase tracking-wider">★ Premium — shown first</span>
        <span class="text-[10px] font-black px-2.5 py-1 rounded-full bg-purple-100 text-purple-700 uppercase tracking-wider">◆ Featured — shown second</span>
        <span class="text-[10px] font-black px-2.5 py-1 rounded-full bg-slate-100 text-slate-500 uppercase tracking-wider">Standard — shown after</span>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden">
        ...
        <div class="grid grid-cols-12 gap-4 px-6 py-4 border-b border-slate-100 last:border-0 items-center hover:bg-slate-50/50 transition-colors">
            ...
            <div class="col-span-4 flex items-center gap-3">
                ...
                @endif
                <div class="min-w-0">
                    <p class="text-sm font-black text-slate-900 uppercase italic tracking-tight leading-tight truncate">{{ $ad->title }}</p>
                    @if($ad->car)
                        <p class="text-[11px] text-slate-400 font-medium mt-0.5 truncate">→ {{ $ad->car->displayName() }}</p>
                    @endif
                </div>
            </div>
            ...
            <div class="col-span-2">
                <span class="text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-wider bg-purple-50 text-purple-700 inline-block leading-tight">
                    {{ $ad->placementLabel() }}
                </span>
            </div>
            ...
            <div class="col-span-1">
                <span class="text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-wider inline-block {{ $ad->priorityBadgeClass() }}">
                    {{ $ad->priorityLabel() }}
                </span>
            </div>
