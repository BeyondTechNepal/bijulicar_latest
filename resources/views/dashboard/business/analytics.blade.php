...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <div class="bg-blue-50 border border-blue-100 rounded-2xl p-5 col-span-2 lg:col-span-1">
        ...
        <p class="text-[11px] text-blue-600/70 font-medium mt-1">{{ number_format($totalRentalDays) }} days rented out</p>
    </div>
...
<div class="bg-white border border-slate-200 rounded-2xl overflow-hidden mb-3">
    ...
    <a href="{{ route('business.rentals.show', $rental) }}"
        ...
        <div class="col-span-4 flex items-center gap-3">
            ...
            @else
            <div class="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center text-[10px] font-black text-slate-400 uppercase shrink-0">
                —
            </div>
            @endif
            <div>
        ...
        <div class="col-span-2">
            ...
            <p class="text-[11px] text-slate-400 font-medium mt-0.5">{{ $rental->renter_phone }}</p>
        </div>

        <div class="col-span-3">
            <p class="text-[11px] text-slate-700 font-semibold">{{ $rental->pickup_date->format('d M Y') }}</p>
            <p class="text-[11px] text-slate-400 font-medium">→ {{ $rental->return_date->format('d M Y') }}</p>
        </div>

        <div class="col-span-1">
            <p class="text-sm font-black text-slate-800">{{ $rental->total_days }}d</p>
        </div>
...
<div class="mb-8 text-right">
    <a href="{{ route('business.rentals.index') }}" class="text-[10px] font-black text-blue-600 uppercase tracking-widest hover:underline">View All Rentals →</a>
</div>
@else
<div class="bg-white border border-dashed border-slate-200 rounded-2xl p-10 text-center mb-8">
    <p class="text-4xl mb-3">🚗</p>
    <p class="font-black text-slate-900 uppercase italic tracking-tight">No rentals yet</p>
    <p class="text-sm text-slate-500 font-medium mt-1">Rental bookings will appear here once renters start booking your vehicles.</p>
</div>
@endif
