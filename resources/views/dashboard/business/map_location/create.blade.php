...
<div class="max-w-2xl">
    ...
    <div class="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
            ...
            <div class="absolute top-3 left-3 right-3 z-[1]">
                <div class="flex gap-2">
                    <input type="text" id="search-input" placeholder="Search address…"
                        class="flex-1 px-4 py-2.5 rounded-xl bg-white shadow-md border border-slate-200 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#a855f7]" />
                    <button onclick="searchAddress()"
                        class="px-4 py-2.5 bg-[#a855f7] hover:bg-[#9333ea] text-white rounded-xl text-sm font-black shadow-md transition-all">
                        Search
                    </button>
                </div>
            </div>
        ...
        <div class="p-6 border-t border-slate-100">
            ...
            <form method="POST" action="{{ route('business.location.store') }}">
                @csrf
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="address" id="address" value="{{ old('address') }}"
                            ...
                            class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#a855f7] @error('address') border-red-400 @enderror" />
                        @error('address')
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="latitude" id="latitude"
                                ...
                                value="{{ old('latitude') }}"
                                placeholder="27.7172"
                                class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm font-mono font-medium focus:outline-none focus:ring-2 focus:ring-[#a855f7] @error('latitude') border-red-400 @enderror" />
                            @error('latitude')
                                <p class="text-red-500 text-xs font-bold mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="longitude" id="longitude"
                                ...
                                value="{{ old('longitude') }}"
                                placeholder="85.3240"
                                class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm font-mono font-medium focus:outline-none focus:ring-2 focus:ring-[#a855f7] @error('longitude') border-red-400 @enderror" />
                            @error('longitude')
                                <p class="text-red-500 text-xs font-bold mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="flex items-center gap-3 mt-6 pt-5 border-t border-slate-100">
                    <button type="submit" id="submit-btn" disabled
                        class="px-6 py-2.5 bg-[#a855f7] hover:bg-[#9333ea] disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-black uppercase tracking-wider rounded-xl transition-all">
                        Save Location
                    </button>
