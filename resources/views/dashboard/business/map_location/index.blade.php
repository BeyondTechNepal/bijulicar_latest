...
<div class="flex items-center justify-between mb-6">
    <p class="text-slate-500 text-sm font-medium">Pin your business on the public map so customers can find you.</p>
    @if (!$location)
        <a href="{{ route('business.location.create') }}"
            class="inline-flex items-center gap-2 px-5 py-2.5 bg-[#a855f7] hover:bg-[#9333ea] text-white text-sm font-black uppercase tracking-wider rounded-xl transition-all shadow-sm">
...
@if (session('success'))
    <div class="mb-6 px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-700 text-sm font-bold">
        ✓ {{ session('success') }}
    </div>
@endif
...
@if (!$location)
    <div class="bg-white border border-dashed border-slate-300 rounded-3xl p-16 flex flex-col items-center justify-center text-center">
        <div class="w-14 h-14 bg-[#a855f7]/10 rounded-2xl flex items-center justify-center mb-4 border border-[#a855f7]/20">
        ...
        <a href="{{ route('business.location.create') }}"
            class="mt-6 inline-flex items-center gap-2 px-6 py-2.5 bg-[#a855f7] hover:bg-[#9333ea] text-white text-sm font-black uppercase tracking-wider rounded-xl transition-all">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
            </svg>
            Pin My Business
            ...
            <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        {{-- Businesses always go live immediately --}}
                ...
                <div class="flex gap-3 shrink-0">
                    <a href="{{ route('business.location.edit') }}"
                    ...
                    <form method="POST" action="{{ route('business.location.destroy') }}"
                        onsubmit="return confirm('Remove your pin from the map?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
