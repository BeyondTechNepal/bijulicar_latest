        ...
        <div class="grid grid-cols-12 gap-4 px-6 py-4 border-b border-slate-100 last:border-0 items-center hover:bg-slate-50/50 transition-colors">
            ...
            <div class="col-span-1">
                @if($rental->isCompleted())
                    ...
                    @if($existingReview)
                        {{-- Already reviewed: show star rating --}}
                        <div class="flex items-center gap-0.5" title="Your rating: {{ $existingReview->rating }}/5">
                            @for($i = 1; $i <= 5; $i++)
                                <svg class="w-3 h-3 {{ $i <= $existingReview->rating ? 'text-yellow-400' : 'text-slate-200' }}" fill="currentColor" viewBox="0 0 20 20">
