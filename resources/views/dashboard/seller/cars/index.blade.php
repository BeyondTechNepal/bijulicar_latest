...
@section('content')
    ...
    <div>
        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">
            {{ ucfirst($prefix) }} Portal
        </p>
        <p class="text-sm font-bold text-slate-600 mt-0.5">
            All vehicles you have listed on BijuliCar.
        </p>
    </div>
    ...
    <div class="border-b border-slate-100 last:border-0 
        ...
        <div class="flex items-center justify-between md:block md:col-span-3">
            ...
            <div>
                ...
                @if($car->price_negotiable)
                    <p class="text-[10px] font-black text-[#16a34a] uppercase tracking-widest mt-0.5">
                        Negotiable
                    </p>
                @endif
            </div>
        ...
        <div class="flex items-center justify-between md:block md:col-span-3">
            ...
            <p class="text-[10px] text-slate-400 font-medium">
                order{{ $car->orders_count !== 1 ? 's' : '' }}
            </p>
        </div>

        {{-- Actions --}}
        <div class="flex justify-end items-center gap-2 md:col-span-2">
            ...
            <form method="POST" action="{{ route($prefix . '.cars.destroy', $car) }}"
                ...
                <button type="submit"
                    class="w-9 h-9 bg-red-50 hover:bg-red-500 rounded-xl flex items-center justify-center transition-all group"
                    title="Delete">
