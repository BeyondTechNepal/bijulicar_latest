...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <a href="{{ route($prefix . '.negotiations.index') }}" class="text-[11px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 transition-colors">← Back to Negotiations</a>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div class="xl:col-span-2 space-y-4">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6">
                ...
                <div class="mt-4 pt-4 border-t border-slate-100 grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <p class="text-base font-black text-slate-500 mt-0.5">NRs {{ number_format($negotiation->listed_price) }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Buyer's Offer</p>
                        <p class="text-base font-black text-slate-900 mt-0.5">NRs {{ number_format($negotiation->offered_price) }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Discount Ask</p>
                        <p class="text-base font-black text-red-500 mt-0.5">-{{ $negotiation->discountPercent() }}%</p>
                    </div>
                </div>

                @if($negotiation->message)
                <div class="mt-4 pt-4 border-t border-slate-100">
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Buyer's Message</p>
            ...
            {{-- ── SELLER ACTIONS ── --}}
            @if($negotiation->isPendingSeller())
            <div class="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Respond to this offer</p>

                {{-- Accept --}}
                <form method="POST" action="{{ route($prefix . '.negotiations.accept', $negotiation) }}">
                    @csrf @method('PATCH')
                    <button type="submit"
                ...
                <div>
                    ...
                    <div id="counterForm" class="hidden mt-3 bg-slate-50 border border-slate-200 rounded-xl p-4">
                        <form method="POST" action="{{ route($prefix . '.negotiations.counter', $negotiation) }}">
                            @csrf @method('PATCH')
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5">Your Counter Price (NRs)</label>
                            <input type="number" name="offered_price" required min="1" max="{{ $negotiation->listed_price - 1 }}"
                                placeholder="e.g. {{ number_format($negotiation->listed_price * 0.95, 0, '.', '') }}"
                            ...
                            <textarea name="message" rows="2" placeholder="Optional message to buyer..."
                ...
                {{-- Decline --}}
                <form method="POST" action="{{ route($prefix . '.negotiations.decline', $negotiation) }}">
                    @csrf @method('PATCH')
                    <button type="submit"
                        onclick="return confirm('Decline this offer? The buyer will be notified.')"
                        class="w-full py-3 rounded-xl bg-red-50 border border-red-200 text-red-600 text-[12px] font-black uppercase italic tracking-widest hover:bg-red-100 transition-all">
                        ✕ Decline Offer
                    </button>
                </form>
            </div>

            @elseif($negotiation->isPendingBuyer())
            <div class="bg-white border border-slate-200 rounded-2xl p-6 text-center py-8">
                <p class="text-2xl mb-2">⏳</p>
                <p class="text-sm font-black text-slate-900">Waiting for buyer to respond</p>
                <p class="text-[11px] text-slate-400 font-medium mt-1">You'll be notified when the buyer accepts or counters.</p>
            </div>
            ...
            <div class="bg-green-50 border border-green-200 rounded-2xl p-6 text-center">
                ...
                <p class="text-[11px] text-green-600 font-medium mt-1">The buyer has been notified and can now place their order at this price.</p>
            </div>

            @elseif($negotiation->isDeclined())
            <div class="bg-slate-50 border border-slate-200 rounded-2xl p-6 text-center">
                <p class="text-sm font-black text-slate-500">You declined this offer.</p>
            </div>

            @elseif($negotiation->isCancelled())
            <div class="bg-slate-50 border border-slate-200 rounded-2xl p-6 text-center">
                <p class="text-sm font-black text-slate-500">The buyer withdrew this offer.</p>
            </div>
            @endif

        </div>

        {{-- ── RIGHT: buyer + car info ─────────────────────────── --}}
        <div class="space-y-4">

            <div class="bg-white border border-slate-200 rounded-2xl p-5">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Buyer</p>
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-slate-900 flex items-center justify-center text-white text-[12px] font-black uppercase shrink-0">
                        {{ strtoupper(substr($negotiation->buyer->name ?? '?', 0, 2)) }}
                    </div>
                    <div>
                        <p class="text-sm font-black text-slate-900">{{ $negotiation->buyer->name ?? 'Unknown' }}</p>
                        <p class="text-[11px] text-slate-400 font-medium">{{ $negotiation->buyer->email ?? '' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-5">
                ...
                <p class="text-base font-black text-slate-800 mt-2">NRs {{ number_format($negotiation->car->price) }}</p>
