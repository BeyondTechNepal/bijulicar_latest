        ...
        <aside id="sidebar"
            ...
            <div class="px-5 py-5 border-b border-slate-800 flex items-center justify-between">
                <a href="{{ route('home') }}" class="flex items-center gap-2">
                    <div
                        class="w-8 h-8 bg-emerald-500 rounded-lg flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.4)]">
                    ...
                    <span class="text-white font-black tracking-tighter uppercase text-sm">Bijuli<span
                            class="text-emerald-500">Car</span></span>
                </a>
            ...
            <div class="px-5 py-4 border-b border-slate-800 bg-slate-800/30">
                ...
                <span
                    class="inline-block mt-1 text-[9px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                    EV Partner
                </span>
            </div>
            ...
            <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
                ...
                <a href="{{ route('station.dashboard') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ request()->routeIs('station.dashboard') ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                ...
                <a href="{{ route('profile.edit') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ request()->routeIs('profile.edit') ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                    <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('station.location.index') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ request()->routeIs('station.location.*') ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                    <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('station.slots.index') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ request()->routeIs('station.slots.*') ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                    <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    Slot Manager
                    @php
                        $pendingSlots = \App\Models\EvStationSlot::where('user_id', auth()->id())
                    ...
                    @if ($pendingSlots)
                        <span
                            class="ml-auto text-[9px] bg-amber-400 text-amber-900 px-1.5 py-0.5 rounded-full font-black">{{ $pendingSlots }}</span>
                    @endif
                </a>

                <p class="text-[9px] font-black text-slate-600 uppercase tracking-widest px-3 py-2 mt-4">Explore</p>
                <div class="px-3 py-2 text-[10px] font-bold text-slate-500 italic">No new modules</div>
            </nav>
        ...
        <div class="flex-1 lg:ml-64 flex flex-col min-w-0">
            ...
            <header
                ...
                <div class="flex items-center gap-4">
                    <span
                        class="flex items-center gap-2 text-[9px] sm:text-[10px] font-black uppercase text-emerald-600 bg-emerald-50 px-2 sm:px-3 py-1.5 rounded-lg border border-emerald-100">
                        <span class="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span>
                        <span class="hidden sm:inline">Network Active</span>
