        ...
        <section
            ...
            @if ($errors->any())
                <div class="mb-4 bg-red-50 border border-red-200 rounded-xl p-4">
                    <p class="text-xs font-bold text-red-700 uppercase tracking-wider mb-1">Validation errors:</p>
            ...
            <form method="POST" action="{{ route('station.verify.store') }}" enctype="multipart/form-data"
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="station_name" value="{{ old('station_name') }}" required
                        ...
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-sm focus:outline-none focus:border-emerald-500 focus:bg-white transition-all font-medium @error('station_name') border-red-400 bg-red-50 @enderror">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-1">
                        ...
                        <input type="text" name="contact_number" value="{{ old('contact_number') }}" required
                            ...
                            class="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-sm focus:outline-none focus:border-emerald-500 focus:bg-white transition-all font-medium">
                    </div>
                    <div class="space-y-1">
                ...
                <div class="space-y-1">
                    ...
                    <label class="block cursor-pointer group">
                        ...
                        <div id="license_label"
                            class="w-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl py-6 px-4 text-center transition-all group-hover:border-emerald-500 group-hover:bg-emerald-50/20">
                            <div class="text-2xl mb-1">⚡</div>
                            <p class="text-sm font-bold text-slate-500 group-hover:text-emerald-600">Click to upload
                                Operating License</p>
                            <p class="text-[10px] text-slate-400 mt-0.5">Electrical inspection certificate or business
                                permit</p>
                        </div>
                    </label>
                </div>
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-4">
                    <p class="text-[11px] font-bold text-emerald-400 uppercase tracking-wider mb-1">Network Protocol</p>
                    <p class="text-xs text-slate-400">By submitting, you agree to provide real-time uptime status to the
                        Bijulicar network once verified.</p>
                </div>

                {{-- Submit --}}
                <button type="submit"
                    class="w-full py-4 bg-emerald-600 text-white rounded-xl font-black uppercase italic tracking-widest text-xs hover:bg-emerald-500 transition-all flex items-center justify-center gap-3 shadow-xl group">
        ...
        function updateFileName(input, labelId) {
            ...
            if (input.files && input.files[0]) {
                ...
                label.innerHTML = `
                    ...
                    <p class="text-[10px] text-slate-400 mt-0.5">${sizeMB} MB — Uplink ready</p>
                `;
