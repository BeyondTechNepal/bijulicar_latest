            ...
            <div class="p-10 text-center">
                ...
                <div class="relative flex items-center justify-center mb-8">
                    @if ($status === 'pending')
                        ...
                        <div class="relative w-16 h-16 bg-amber-50 border-2 border-amber-200 rounded-2xl flex items-center justify-center">
                            <svg class="w-8 h-8 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                        </div>
                    @elseif ($status === 'rejected')
                        <div class="relative w-16 h-16 bg-red-50 border-2 border-red-200 rounded-2xl flex items-center justify-center">
                            <svg class="w-8 h-8 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                        </div>
                    @else
                        <div class="relative w-16 h-16 bg-slate-50 border-2 border-slate-200 rounded-2xl flex items-center justify-center">
                ...
                @if ($status === 'pending')
                    ...
                    <p class="text-slate-500 text-sm font-medium leading-relaxed max-w-sm mx-auto">
                        Your details have been submitted successfully. Our admin team is reviewing your information and documents. You'll receive an email once a decision is made.
                    </p>
                    ...
                    <div class="mt-6 bg-slate-50 border border-slate-200 rounded-2xl p-5 text-left space-y-3">
                        ...
                        @if ($user->hasRole('seller') && $user->sellerVerification)
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-xs font-bold text-slate-900">{{ $user->sellerVerification->full_name }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-bold text-slate-500">Contact</span>
                                <span class="text-xs font-bold text-slate-900">{{ $user->sellerVerification->contact }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                        ...
                        @elseif ($user->hasRole('business') && $user->businessVerification)
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-xs font-bold text-slate-900">{{ $user->businessVerification->business_name }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-bold text-slate-500">Contact</span>
                                <span class="text-xs font-bold text-slate-900">{{ $user->businessVerification->contact }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                ...
                @elseif ($status === 'rejected')
                    ...
                    <p class="text-slate-500 text-sm font-medium leading-relaxed max-w-sm mx-auto">
                        Unfortunately your verification application was not approved. Please review the reason below, fix the issue, and resubmit.
                    </p>

                    {{-- Rejection reason --}}
                    @if ($verification->rejection_reason)
                        <div class="mt-6 bg-red-50 border border-red-200 rounded-2xl p-5 text-left">
                            <p class="text-[10px] font-black text-red-500 uppercase tracking-widest mb-2">Reason for rejection</p>
                            <p class="text-sm text-red-700 font-medium leading-relaxed">{{ $verification->rejection_reason }}</p>
                        </div>
                    @endif
    ...
    <a href="{{ $resubmitRoute }}"
        ...
        <svg class="w-4 h-4 group-hover:rotate-180 transition-transform duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                ...
                @else
                    ...
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2">Action Required</p>
                    <h1 class="text-2xl font-black text-slate-900 uppercase italic tracking-tight mb-3">
                    ...
                    <p class="text-slate-500 text-sm font-medium leading-relaxed max-w-sm mx-auto mb-6">
                        You haven't submitted your verification details yet. Complete the form to get your account approved.
                    </p>
        ...
        <div class="text-center mt-6">
            <form action="{{ route('logout') }}" method="POST" class="inline">
                @csrf
                <button type="submit" class="text-[11px] font-bold text-slate-400 uppercase tracking-widest hover:text-slate-600 transition-colors">
